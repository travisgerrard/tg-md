...
export default class BugEdit extends React.Component {
  ...
  loadData() {
    ...
    fetch(`http://localhost:3000/api/bugs/${this.props.params.id}`).then(response => response.json()).then(bug => {
  ...
  submit(e) {
    ...
    fetch(`http://localhost:3000/api/bugs/${this.props.params.id}`, {
      method: 'PUT',
  ...
  render() {
    const success = (
      <Alert bsStyle="success" onDismiss={this.dismissAlert} >Bug saved to DB successfully</Alert>
    );
        ...
        <Panel header={`Edit bug: ${this.props.params.id}`}>
          <form onSubmit={this.submit}>
            <FormGroup>
              ...
              <FormControl componentClass="select" name="priority" value={bug.priority} onChange={this.onChange}>
            ...
            <FormGroup>
              <ControlLabel>Status</ControlLabel>
                <FormControl componentClass="select" name="status" value={bug.status} onChange={this.onChange}>
                  <option>New</option>
                  <option>Open</option>
                  <option>Fixed</option>
            ...
            <FormGroup>
              <ControlLabel>Ownere</ControlLabel>
              <FormControl type="text" name="owner" value={bug.owner} onChange={this.onChange} />
            </FormGroup>
            <ButtonToolbar>
              <Button type="submit" bsStyle="primary">Submit</Button>
              <Link className="btn" to="/bugs">Back</Link>
            </ButtonToolbar>
          </form>
        </Panel>
